...
export default function App(props) {
  ...
  const login = async (username, password) => {
    let res = await API.post(endpoints['login'], {
      'client_id': 'HgTmfMKxi7DVfdAwXmaTsUxXdkvAMgZ3UCmQShZi',
      'client_secret': 'WfTM1TxkRvkFbaXzzseqIBcsX6LtLazLzKVC8bg9uzcdaurNqw0pOkyCaXjJtrnzQn6v0YaiJk82cRNDVpl10ldJKanimuxLOrlSZh1H8NH038pQbxOc1bzxQOSF82DK',
      'username': username,
      'password': password,
      'grant_type': 'password'
    })
        ...
        <>
          <Modal show={lgShow} onHide={handleClose}
            ...
            <Modal.Header closeButton id="example-modal-sizes-title-lg">
              <Modal.Title className="text-center">Chi tiết đặc tiệc</Modal.Title>
            </Modal.Header>
            <Modal.Body>
...
function MyVerticallyCenteredModal(props) {
  return (
    ...
    >
      ...
      <Modal.Body >
        {/* {props.Re } */}
        {(props.Re == undefined || props.Re !== true) ? <A_login hideForm={props.onHide} /> : <A_Register hideForm={props.onHide}/>}

        {/* {`${if(props.Regis == 'Register') 
            <A_Register/>
          else 
            <A_login hideForm={props.onHide}/>}`} */}
